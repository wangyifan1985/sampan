...
class BasicDecoder(Decoder):
    ...
    def decode(self, s: str) -> Any:
        if not s or not isinstance(s, str):
            raise JsonDecodeError(s, 0, 'Only "str" type is acceptable: ')
        pos = self.utf8_bom_re.match(s).end()
        obj, pos = self.scan(s, pos)
        if self.skip_whitespace(s, pos) != len(s):
            raise JsonDecodeError(s, pos, 'Incorrect end of json string: ')
        return obj
    ...
    def decode_string(self, s: str, pos: int) -> Tuple[str, int]:
        ...
        while True:
            ...
            if m is None:
                raise JsonDecodeError(s, pos, 'Can not decode json "string" string: ')
            chunk, term = m.groups()
            if chunk:
                _string.write(chunk)
            end = m.end()
    ...
    def decode_array(self, s: str, pos: int) -> Tuple[List[Any], int]:
        ...
        while True:
            ...
            if s[end] == ']':
                ...
                raise JsonDecodeError(s, pos, 'Can not decode json "array" string: ')
        return _array, end + 1
    ...
    def decode_object(self, s: str, pos: int) -> Tuple[Dict[str, Any], int]:
        ...
        while True:
            ...
            if s[end] != ':':
                raise JsonDecodeError(s, pos, 'Can not decode json "object" string: ')
...
class DefaultFormatter(Formatter):
    ...
    def format(self, s: str):
        if not s or not isinstance(s, str):
            raise JsonFormatError(s, 0, 'Only "str" type is acceptable: ')
        js = StringIO()
        ...
        if _default_decoder.skip_whitespace(s, pos) != len(s):
            raise JsonFormatError(s, 0, 'Incorrect end of json string: ')
        js.write(s[pos:])
        return js.getvalue()
    ...
    def format_null(self, s: str, pos: int, align: int) -> Tuple[str, int]:
        ...
        if m is None:
            raise JsonFormatError(s, pos, 'Can not format json "null" string: ')
    ...
    def format_boolean(self, s: str, pos: int, align: int) -> Tuple[str, int]:
        ...
        if m is None:
            raise JsonFormatError(s, pos, 'Can not format json "boolean" string: ')
        return self.concat(align, m.group()), m.end()

    def format_number(self, s: str, pos: int, align: int) -> Tuple[str, int]:
        m = _default_decoder.number_re.match(s, pos)
        if m is None:
            raise JsonFormatError(s, pos, 'Can not format json "number" string: ')
        return self.concat(align, m.group()), m.end()
    ...
    def format_string(self, s: str, pos: int, align: int) -> Tuple[str, int]:
        ...
        while True:
            ...
            if m is None:
                raise JsonFormatError(s, pos, 'Can not format json "string" string: ')
            _, term = m.groups()
            end = m.end()
            if term == '"':
                break
    ...
    def format_array(self, s: str, pos: int, align: int) -> Tuple[str, int]:
        ...
        while True:
            ...
            if s[end] == ']':
                ...
                raise JsonFormatError(s, pos, 'Can not format json "array" string: ')
        align -= self.indent
    ...
    def format_object(self, s: str, pos: int, align: int) -> Tuple[str, int]:
        ...
        while True:
            ...
            if s[end] != ':':
                raise JsonFormatError(s, pos, 'Can not format json "object" string: ')
